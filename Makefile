...
kind-create-cluster: ## Create Kind cluster
	...
	@printf "$(call PRINT_COLOR,Setting kubectl context to "kind-kind", default namespace to "microservices"\n)"
...
kind-build-images: ## Build Docker images for Microservices
	...
	@for service in $(MICROSERVICES); do \
		...
		docker build --target production -t microservices/$$svc:$(VERSION) .; \
		docker image tag microservices/$$svc:$(VERSION) $(DOCKER_REGISTRY)/$$svc:$(VERSION); \
		\
		cd - > /dev/null; \
	done
...
kind-deploy-services: ## Deploy all services to Kind
	...
	@kubectl apply -f ./k8s/rabbitmq/configmap.yaml || true
	@kubectl apply -f ./k8s/rabbitmq/secret.yaml || true
	@kubectl apply -f ./k8s/rabbitmq/headless-service.yaml || true
	@kubectl apply -f ./k8s/rabbitmq/statefulset.yaml || true

	@printf "$(call PRINT_COLOR,Deploying Microservices\n)"

	@for service in $(MICROSERVICES); do \
		printf "$(call PRINT_COLOR,Deploying $$service\n)"; \
	  kubectl apply -f ./k8s/$$service/configmap.yaml || true; \
		kubectl apply -f ./k8s/$$service/secret.yaml || true; \
		kubectl apply -f ./k8s/$$service/service.yaml || true; \
		kubectl apply -f ./k8s/$$service/deployment.yaml || true; \
	done

	@printf "$(call PRINT_COLOR,Deploying API Gateway Ingress\n)"
